...
# Model parameters
n <- 6  # Number of items
N <- 30  # Number of total orders to sample
...
# Create design matrix (p x n, where p is number of covariates)
...
synthetic_data <- generate_synthetic_data(
  ...
  beta_true = beta_true,
  X = t(X),  # transpose to make it n x p for the function
  random_seed = 42
)
...
```{r plot-true-po, fig.width=8, fig.height=6}
# Create visualization of the true partial order with clean styling
plot_partial_order(h_true, items, title = "True Partial Order", 
                  vertex_size = 25, edge_arrow_size = 0.4,
...
```{r fixed-mcmc}
# MCMC parameters
num_iterations <- 5000  # Reduced for faster demonstration
burn_in <- 200

# Update probabilities (must sum to 1)
mcmc_pt <- c(0.2, 0.2, 0.4, 0.2)  # rho, noise, U, beta
...
mcmc_results <- mcmc_partial_order(
  ...
  K = K,
  X = X,
  dr = 0.1,                    # Multiplicative step for rho
  drbeta = 0.1,               # Step size for beta
  sigma_mallow = 0.1,         # Mallows parameter step
...
cat("MCMC completed with acceptance rate:", 
    round(mcmc_results$overall_acceptance_rate * 100, 2), "%\n")
```
...
```{r rj-mcmc}
# Reversible Jump MCMC parameters
mcmc_pt_rj <- c(0.15, 0.15, 0.3, 0.15, 0.25)  # rho, noise, U, beta, K

# Prior for K (Poisson with rate K_prior)
K_prior <- 3.0

# Run Reversible Jump MCMC
rj_results <- mcmc_partial_order_k(
  observed_orders = observed_orders,
  choice_sets = choice_sets,
  num_iterations = num_iterations,
  X = X,
...
cat("Reversible Jump MCMC completed with acceptance rate:", 
    round(rj_results$overall_acceptance_rate * 100, 2), "%\n")
```
...
burn_indices <- (burn_in/100 + 1):length(mcmc_results$rho_trace)
rj_burn_indices <- (burn_in/100 + 1):length(rj_results$rho_trace)

# Fixed dimension estimates
rho_est_fixed <- mean(mcmc_results$rho_trace[burn_indices])
beta_est_fixed <- colMeans(do.call(rbind, mcmc_results$beta_trace[burn_indices]))
...
```{r trace-plots, fig.width=12, fig.height=10}
...
plot_mcmc_results(mcmc_results, 
...
# Plot K trace with burn-in
plot(rj_results$K_trace, type = "l", main = "K Evolution Over Time",
     xlab = "Iteration (x100)", ylab = "K", col = "darkgreen")
abline(v = burn_in/100, col = "orange", lty = 2, lwd = 2)
abline(h = K, col = "red", lty = 2, lwd = 2)
legend("topright", c("True K", "Burn-in"), col = c("red", "orange"), lty = 2)
...
if (length(mcmc_results$log_likelihood_currents) > 0 && 
    all(is.finite(mcmc_results$log_likelihood_currents))) {
  plot(mcmc_results$log_likelihood_currents, type = "l", 
...
if (length(mcmc_results$acceptance_rates) > 0 && 
    all(is.finite(mcmc_results$acceptance_rates))) {
  plot(mcmc_results$acceptance_rates, type = "l", 
...
h_trace_fixed_post_burnin <- mcmc_results$h_trace[(burn_in_index + 1):length(mcmc_results$h_trace)]
...
# True partial order
plot_partial_order(h_true, items, title = "True Partial Order",
                  vertex_size = 20, edge_arrow_size = 0.3,
                  vertex_color = "lightgray", 
                  vertex_frame_color = "darkgray",
                  edge_color = "darkblue")

# Fixed K estimate (using posterior mean)
plot_partial_order(final_h_fixed, items, title = "Fixed K MCMC Estimate\n(Posterior Mean)",
                  vertex_size = 20, edge_arrow_size = 0.3,
                  vertex_color = "lightcoral", 
                  vertex_frame_color = "darkred",
                  edge_color = "darkred")

# RJ-MCMC estimate (using posterior mean)
plot_partial_order(final_h_rj, items, title = "RJ-MCMC Estimate\n(Posterior Mean)",
                  vertex_size = 20, edge_arrow_size = 0.3,
                  vertex_color = "lightgreen", 
                  vertex_frame_color = "darkgreen",
...
The `mcmc_partial_order_k` function implements the following update
steps:

1.  **Rho Update**: Multiplicative random walk with step size `dr`
2.  **Noise Parameter Update**:
    -   Queue-jump: Beta proposal
    -   Mallows: Log-normal random walk
3.  **Latent Matrix Update**: Single row updates with multivariate
    normal proposals
4.  **Beta Update**: Component-wise normal random walk
5.  **Dimension Update**: Birth/death moves for K

```{r mcmc-details}
# Example of manual parameter tuning
tuned_mcmc_pt <- c(0.1, 0.1, 0.4, 0.2, 0.2)  # Reduce K update frequency

# Run with different tuning
tuned_results <- mcmc_partial_order_k(
  observed_orders = observed_orders,
  choice_sets = choice_sets,
  num_iterations = 5000,  # Shorter run for demonstration
  X = X,
  dr = 1.05,              # Smaller step for rho
  drbeta = 0.05,          # Smaller step for beta
  sigma_mallow = 0.05,    # Smaller step for Mallows
  sigma_beta = sigma_beta,
  noise_option = "queue_jump",
  mcmc_pt = tuned_mcmc_pt,
  rho_prior = rho_prior,
  noise_beta_prior = noise_beta_prior,
  mallow_ua = 1.0,
  K_prior = K_prior,
...
cat("Tuned MCMC acceptance rate:", 
    round(tuned_results$overall_acceptance_rate * 100, 2), "%\n")
```

## Mallows Noise Model

```{r mallows-example}
# Example with Mallows noise model
mallows_results <- mcmc_partial_order_k(
  observed_orders = observed_orders,
  choice_sets = choice_sets,
  num_iterations = 5000,
  X = X,
  ...
  sigma_mallow = 0.1,
  sigma_beta = sigma_beta,
  noise_option = "mallows_noise",  # Switch to Mallows
  mcmc_pt = mcmc_pt_rj,
  rho_prior = rho_prior,
  noise_beta_prior = noise_beta_prior,
  mallow_ua = 1.0,
...
cat("Mallows MCMC acceptance rate:", 
    round(mallows_results$overall_acceptance_rate * 100, 2), "%\n")
```
